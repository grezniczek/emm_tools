...
const EMDT = window.DE_RUB_EMDTools ?? {
    ...
    toggleShowFieldAnnotations: toggleShowFieldAnnotations
};
// @ts-ignore
...
function toggleShowFieldAnnotations() {
    const $state = $('#emdt-fieldannotations-state')
    if ($state.attr('working') == '1') return
    $state.html('<i class="fas fa-spinner fa-spin"></i>').attr('working','1')
    JSMO.ajax('toggle-show-field-annotations')
    .then((data) => {
        log('Toggled "Show Field Annotations');
        $state.text(data);
    })
    .catch((err) => {
        error('Failed to toggle "Show Field Annotations" state:', err);
    })
    .finally(() => {
